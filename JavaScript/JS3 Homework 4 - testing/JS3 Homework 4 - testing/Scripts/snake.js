...
var snakeGame = (function() {
  ...
  var SnakeGame = Class.create({
    ...
    checkCollisions: function() {
        this.translateAtBorder();
        for (var i = 0; i < this.gameObjects.length; i += 1) {
            var gameObject = this.gameObjects[i];
            if (gameObject instanceof Snake) {
                // gameObject is the snake itself
                // then check for collisions with the tail pieces
                // obviously the snake cannot bite its first two pieces
                for (var j = 3; j < gameObject.pieces.length; j++) {
                    var bitting = this.snake.intersects(gameObject.pieces[j]);
                    if (bitting) {
                        this.snake.consume(gameObject.pieces[j]);
                    }
                }
            } else {
                ...
                if (colliding) {
                    ...
                    this.putNewFoodObject(gameObject);
                }
            }
        }
    },
    translateAtBorder: function () {
    ...
    putNewFoodObject: function(gameObject) {
    ...
    start: function() {
        ...
        // Initially this die handler was attached only on game initialization
        // and after restarting the snake became immortal
