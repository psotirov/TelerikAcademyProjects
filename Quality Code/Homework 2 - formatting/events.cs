...
// Please note that only formatting rules are applied as per the homework requirements
...
class Program
{
    ...
    class EventHolder
    {
        ...
        {
            OrderedBag<Event>.VieweventsToShow = byDate.RangeFrom(new Event(date, string.Empty, string.Empty), true);
            int showed = 0;
    ...
    private static bool ExecuteNextCommand()
    {
		...
		// series of if statements were exchanged by single switch
        switch (command[0])
        {
            case 'A':
                AddEvent(command);
                return true;
				// in this case break operator could be omitted (after return operator)
            case 'D':
